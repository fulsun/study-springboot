...
import tk.fulsun.demo.exception.WrapMessageException;

/**
 * p>枚举类异常断言，提供简便的方式判断条件，并在条件满足时抛出异常
 *
 * <p>错误码和错误信息定义在枚举类中，在本断言方法中，传递错误信息需要的参数
 *
 * @author fulsun
 * @date 7/1/2021
 */
public interface Assert {
  ...
  /**
   * 创建异常. 先使用 {@code errMsg} 创建一个 {@link WrapMessageException} 异常, 再以入参的形式传给 {{{@link
   * #newException(Throwable, Object...)}}}, 作为最后创建的异常的 cause 属性.
   *
  ...
  /**
   * 创建异常. 先使用 {@code errMsg} 和 {@code t} 创建一个 {@link WrapMessageException} 异常, 再以入参的形式传给 {{{@link
   * #newException(Throwable, Object...)}}}, 作为最后创建的异常的 cause 属性.
   *
   * @param errMsg 自定义的错误信息
   * @param args
